...
// kernel01
__global__ void Kernel01 (int width, int height, unsigned int *matrixR, unsigned int *matrixG, unsigned int *matrixB, unsigned char *new_image) {
    
    __shared__ unsigned int sharedR[SIZE][SIZE];
    __shared__ unsigned int sharedG[SIZE][SIZE];
    __shared__ unsigned int sharedB[SIZE][SIZE];
    ...
    for(int m = 0; m < width; m+=SIZE){
    	sharedR[ty][tx] = matrixR[row*width + m + tx];
    	sharedG[ty][tx] = matrixG[row*width + m + tx];
    	sharedB[ty][tx] = matrixB[row*width + m + tx];
        // esperamos a que todos los threads inserten valores en la memoria compartida
        __syncthreads();
        ...
        for (int i = -margin_x; i < (margin_x + 1); i++) {
            for (int j = -margin_y; j < (margin_y + 1); j++) {
                    c = tx + j;
                    r = min(max(0, r), SIZE - 1);
                    c = min(max(0, c), SIZE - 1);
                    unsigned int pixelR = sharedR[r][c];
                    unsigned int pixelG = sharedG[r][c];
                    unsigned int pixelB = sharedB[r][c];
            ...
            // sincornizamos los threads antes de continuar
        __syncthreads();
    }
}
...
int main(int argc, char **argv) {
    ...
    nBlocks = (width+nThreads-1)/nThreads;
